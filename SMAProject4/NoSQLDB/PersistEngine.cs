...
namespace Project4Starter
{
    public class PersistEngine<Key, Value>
    {
        ...
        /*-----------------Function to persist the database contents with list of strings payload to XML doc------------------*/
        public void persistToXMLListPayload(dynamic keys)
        {
            try
            {
                XDocument xmlDoc = new XDocument();
                ...
                foreach (var item in keys)
                {
                    ...
                    value.Add(payload);
                    foreach (var c in elem2.payload)
                    {
                        XElement payloads = new XElement("payloads", c);
                        payload.Add(payloads);
                    }
                }
                saveXML(xmlDoc);
            }
        ...
        public dynamic fetchDBElement(XDocument xmlDoc)
        {
            var values = from y in xmlDoc.Descendants("Record")                                             //fetch values from the xml
                         select new DBElement<string, List<string>>()
                         {
                             name = y.Descendants("Value").Descendants("name").FirstOrDefault().Value,
                             descr = y.Descendants("Value").Descendants("descr").FirstOrDefault().Value,
                             timeStamp = DateTime.Now,
                             children = y.Descendants("Value").Descendants("children").Descendants("child").Select(child => { return (child.Value); }).ToList(),
                             payload = y.Descendants("payload").Descendants("payloads").Select(payloads => { return (payloads.Value); }).ToList(),
                             key = y.Descendants("Key").First().Value,
                         };
            return values;
        }

        /*-------------------Function to augment persisted xml file to the database having list of strings----------*/
        public void augmentDatabaseFromXMLStringList(dynamic db, string fileName)
        {
            try
            {
                XDocument xmlDoc = XDocument.Load(fileName);   //path from where xml has to be loaded
                var values = fetchDBElement(xmlDoc);
                foreach (var value in values)
                {
                    db.insert(value.key, value);                                                                //insert the key/value pairs to the main database
                }
