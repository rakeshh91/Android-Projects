 ...
 * --------------------
 * ver 2.2 : 20 Nov 2015
 * - added latency functionality in defaultServiceAction()
 * ver 2.1 : 29 Oct 2015
...
namespace Project4Starter
{
    ...
    public class Receiver
    {
         ...
         */
        HiResTimer timerRcvr = new HiResTimer();
        public double totalTimeClient { get; set; } = 0;
        public int countRequests { get; set; } = 0;
        public double avgLatency { get; set; } = 0;
        ...
        public void setTimerFromClient(HiResTimer timerClient)
        {
            timerRcvr = timerClient;
        }
        ...
        public Action defaultServiceAction()
        {
            Action serviceAction = () =>
            {
                ...
                while (true)
                {
                    ...
                    timerRcvr.Stop();
                    double totalTimeClient = timerRcvr.ElapsedTimeSpan.TotalSeconds;
                    avgLatency = totalTimeClient / ++countRequests;
                    Console.Write("\n  Received message:");
                    Console.Write("\n================================\n");
                    Console.Write("\n  Sender is:\t {0}", msg.fromUrl);
                    Console.Write("\n  Content is:\t {0}\n", msg.content);
                    Console.Write("\n The average latency is:\t {0}\n", avgLatency);
