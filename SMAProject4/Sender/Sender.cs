 ...
 * --------------------
 * ver 2.2 : 20 Nov 2015
 * - added functions to send the performance measure of the readers, writers and the server
 * ver 2.1 : 29 Oct 2015
...
namespace Project4Starter
{
    ...
    public class Sender
    {
        ...
        //------------send average latency message of the writer---------//
        public void sendLatencyWriter(double avgLatencyWriter)
        {
            if (proxyStore.ContainsKey(remoteUrl))
            {
                proxyStore[remoteUrl].getPerformanceResultsWriter(avgLatencyWriter);
            }
            else
            {
                // create new proxy with Connect, save it, and use it
                ...
                if (this.Connect(remoteUrl))  // if Connect succeeds it will set proxy and send start msg
                {
                    ...
                    proxy.getPerformanceResultsWriter(avgLatencyWriter);
                }
            }
        }

        //-------------------------Send average latency of the reader-----------------//
        public void sendLatencyReader(double avgLatencyReader)
        {
            if (proxyStore.ContainsKey(remoteUrl))
            {
                proxyStore[remoteUrl].getPerformanceResultsReader(avgLatencyReader);
            }
            else
            {
                // create new proxy with Connect, save it, and use it
                ...
                {
                    proxyStore[remoteUrl] = this.proxy;  // save proxy
                    proxy.getPerformanceResultsReader(avgLatencyReader);
                }
            }
        }

        //-------------send performance timing information to WPF
        public TimingInfo sendPerformanceWPF()
        {
            if (proxyStore.ContainsKey(remoteUrl))
            {
                return(proxyStore[remoteUrl].getPerformanceWPF());
            }
            else
            {
                // create new proxy with Connect, save it, and use it
                ...
                {
                    proxyStore[remoteUrl] = this.proxy;  // save proxy
                    return(proxy.getPerformanceWPF());
                }
